...
def forest_plot(per_cluster: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(
        title="Per-cluster effect (B − A) with 95% CI",
        xaxis_title="Mean score difference (points)",
        yaxis_title="",
        height=430,
...
def score_distributions(scores_a: np.ndarray, scores_b: np.ndarray) -> go.Figure:
    ...
    fig = go.Figure()
    fig.add_trace(go.Histogram(x=scores_a, name="Resume A", opacity=0.6,
                               nbinsx=50, marker_color=A_COLOR))
    fig.add_trace(go.Histogram(x=scores_b, name="Resume B", opacity=0.6,
                               nbinsx=50, marker_color=B_COLOR))
    fig.update_layout(
        barmode="overlay",
        title="Match-score distribution across 2,000 jobs",
...
def bayesian_posterior(bayes) -> go.Figure:
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=xs, y=pdf, mode="lines", line_color="#2563eb",
                             name="posterior"))
    mask = (xs >= bayes.ci_low) & (xs <= bayes.ci_high)
    fig.add_trace(go.Scatter(x=xs[mask], y=pdf[mask], fill="tozeroy", mode="none",
                             fillcolor="rgba(37,99,235,0.20)", name="95% CrI"))
    fig.add_vline(x=0.5, line_dash="dash", line_color=A_COLOR,
                  annotation_text="p = 0.5", annotation_position="top")
    fig.update_layout(
...
def sequential_trajectory(seq) -> go.Figure:
    """Always-valid p-value as the 2,000 jobs accumulate (log scale)."""
    n = np.arange(1, seq.n + 1)
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=n, y=np.clip(seq.trajectory_p, 1e-300, 1.0),
                             mode="lines", line_color="#7c3aed",
                             name="always-valid p"))
    fig.add_hline(y=0.05, line_dash="dash", line_color=A_COLOR,
