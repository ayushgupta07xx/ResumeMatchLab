...
def required_sample_size(effect_size: float, alpha: float = 0.05,
    ...
    if effect_size == 0:
        return float("inf")
    return float(_ANALYSIS.solve_power(
...
def achieved_power(n: int, effect_size: float, alpha: float = 0.05) -> float:
    ...
    if effect_size == 0 or n < 2:
        return float(alpha)
    val = _ANALYSIS.power(
        effect_size=abs(effect_size), nobs=n, alpha=alpha,
        alternative="two-sided")
...
def mde_table(n: int, alphas: tuple[float, ...] = (0.01, 0.05, 0.10),
              powers: tuple[float, ...] = (0.80, 0.90, 0.95)) -> pd.DataFrame:
